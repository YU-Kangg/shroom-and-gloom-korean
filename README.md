...
Windows Steam **0.6.39 / 빌드 25342632**용 비공식 유저 한글화입니다. 현재 버전은 **v0.1.3-beta.1**입니다. 0.6.24와 0.6.39에서 추가·변경된 문구를 반영했습니다.

카드 이름뿐 아니라 효과, 키워드에 마우스를 올렸을 때의 설명, 적, 대화와 UI를 번역합니다. 단일 대상 피해는 **‘대상에게 5 피해’**, 전체 공격은 **‘모든 적에게 5 피해’**처럼 표시합니다. 번역 문구 2,749개를 수록했으며, 제작진 이름·기호·의도적인 글자 깨짐 연출 등 77개는 원문을 유지합니다. 모든 카드 조합과 후반 이벤트를 실기 검증한 것은 아닙니다.

## 설치

1. 게임을 종료합니다. 다른 한글 패치나 모드를 사용 중이라면 해당 파일을 먼저 별도 보관합니다. 데모용 XUnity 패치와 함께 설치하지 마세요.
2. Releases에서 `ShroomAndGloom-Korean-v0.1.3-beta.1-win-x64.zip`을 받습니다. GitHub의 `Source code (zip)`은 설치 파일이 아닙니다.
